...
function main() {
    ...
    const light = new THREE.DirectionalLight(0xfefefe, 1);
    light.position.set(23, 45, 50);
    scene.add(light);

    const pointLight = new THREE.PointLight(0xfefefe, .5, 30);
    scene.add(pointLight);

    const ambientLight = new THREE.AmbientLight(0x3F323F);
    scene.add(ambientLight);

    const glbLoader = new GLTFLoader();
    glbLoader.load('../assets/models/victorian-house.glb',
        (glb) => {
            /* indentify meshes wich are part of the ground by looking material name */
            const isGround = (mesh) => /ground|deck/i.test(mesh.name);
            ground = glb.scene.children.filter(isGround);
